...
class MerapiClient {
    constructor(params) {
        ...
        this.getCashOutRequests = async () => {
            const response = await this.http.get('/cashout/pcodes');
            const result = response.data.data.items;
            return result.map(buildCashoutItem);
        };
        this.linkCardToAddress = async (params) => {
            await this.http.post('/cashout/card/link', params);
        };
        this.getAddressByCardHash = async (cardhash) => {
            const response = await this.http.get(`/cashout/card/link?cardhash=${cardhash}`);
            return response.data.data.items[0]?.address;
        };
        this.getLinkedCardsCount = async () => {
            const response = await this.http.post('/cashout/card/link/count');
            return response.data.data.items[0].linked_cards_count;
        };
        this.submitTransaction = async (params) => {
            const response = await this.http.post('/wallet/transaction', params);
            return response.data;
        };
        this.getRpcUrl = async () => {
            const response = await this.http.post('/wallet/service/info');
            return response.data.nodeUrl;
        };
        this.getTransactions = async () => {
            const response = await this.http.get('/wallet/transactions');
            return response.data;
        };
...
function buildCashoutItem(result) {
    return {
        secureCode: result.secure_code,
        pcode: result.pcode,
